...
const DoubanSelector: React.FC<DoubanSelectorProps> = ({
  ...
  const renderCapsuleSelector = (
    ...
    return (
      <div
        ref={containerRef}
        className='relative inline-flex bg-surface-tertiary/60 rounded-full p-0.5 sm:p-1 backdrop-blur-sm'
      >
        {/* 滑动的白色背景指示器 */}
        {indicatorStyle.width > 0 && (
          <div
            className='absolute top-0.5 bottom-0.5 sm:top-1 sm:bottom-1 bg-surface-hover rounded-full shadow-sm transition-all duration-300 ease-out'
            style={{
          ...
          return (
            <button
              ...
              className={`relative z-10 px-2 py-1 sm:px-4 sm:py-2 text-xs sm:text-sm font-medium rounded-full transition-all duration-200 whitespace-nowrap ${
                isActive
                  ? 'text-content-primary cursor-default'
      ...
      {type === 'movie' && (
        <div className='space-y-3 sm:space-y-4'>
          ...
          <div className='flex flex-col sm:flex-row sm:items-center gap-2'>
            <span className='text-xs sm:text-sm font-medium text-content-secondary min-w-[48px]'>
              分类
            </span>
