# 🔄 播放页面重构计划

> V0TV 项目 - 播放页面最佳实践重构

---

## 📋 重构概述

**当前状况**:
- 单文件 1900+ 行
- 逻辑混杂，难以维护
- 类型不安全，过多 any

**目标**:
- ✅ 清晰的文件结构
- ✅ 关注点分离
- ✅ 类型安全
- ✅ 可复用性高
- ✅ 易于测试

---

## 🗂️ 新文件结构

```
src/app/play/
├── page.tsx                          # 主页面入口（~150行）
├── components/
│   ├── VideoPlayer/
│   │   ├── index.tsx                 # 播放器主组件（~200行）
│   │   ├── PlayerControls.tsx       # 自定义控制栏（~100行）
│   │   └── AdFilterLoader.ts        # 广告过滤加载器（~100行）
│   ├── EpisodeList/
│   │   └── index.tsx                 # 选集列表（已存在，需优化）
│   ├── VideoInfo/
│   │   ├── index.tsx                 # 视频信息展示（~150行）
│   │   └── FavoriteButton.tsx       # 收藏按钮（~50行）
│   └── ErrorBoundary/
│       └── index.tsx                 # 错误边界（已完成）
├── hooks/
│   ├── useVideoPlayer.ts             # 播放器逻辑（~250行）
│   ├── usePlaybackHistory.ts        # 播放记录（~150行）
│   ├── useSourceSelection.ts        # 源选择和换源（~200行）
│   ├── useKeyboardShortcuts.ts      # 快捷键（~100行）
│   ├── useFavorite.ts                # 收藏功能（~80行）
│   └── useVideoData.ts               # 视频数据获取（~200行）
├── types/
│   └── player.types.ts               # 播放器类型定义（~100行）
└── utils/
    ├── player.utils.ts               # 播放器工具函数（~100行）
    └── url.utils.ts                  # URL处理工具（~50行）
```

**代码分布**:
- 主页面: ~150行
- 组件: ~700行
- Hooks: ~980行
- 类型+工具: ~250行
- **总计**: ~2080行（比原来多180行，但结构清晰）

---

## 🎯 拆分原则

### 1. 单一职责原则 (SRP)
- 每个组件/Hook只负责一个功能
- 避免"上帝组件"

### 2. 关注点分离
- **UI组件**: 只负责渲染
- **Hooks**: 处理业务逻辑
- **Utils**: 纯函数工具

### 3. 类型安全
- 消除所有 `any` 类型
- 完整的类型定义
- 严格的null检查

### 4. 可测试性
- Hook可独立测试
- 组件props清晰
- 工具函数纯函数

---

## 📦 详细设计

### 1. 类型定义 (`types/player.types.ts`)

```typescript
import { SearchResult } from '@/lib/types';
import Artplayer from 'artplayer';

// 播放器实例类型
export type ArtPlayerInstance = Artplayer;

// 播放器配置
export interface VideoPlayerConfig {
  url: string;
  poster: string;
  title: string;
  autoplay?: boolean;
  volume?: number;
}

// 播放状态
export interface PlaybackState {
  currentTime: number;
  duration: number;
  isPlaying: boolean;
  volume: number;
}

// 视频数据
export interface VideoData {
  detail: SearchResult | null;
  currentSource: string;
  currentId: string;
  currentEpisodeIndex: number;
  videoTitle: string;
  videoYear: string;
  videoCover: string;
}

// 加载状态
export type LoadingStage =
  | 'searching'
  | 'preferring'
  | 'fetching'
  | 'ready'
  | 'sourceChanging';

// 键盘快捷键配置
export interface KeyboardShortcut {
  key: string;
  altKey?: boolean;
  ctrlKey?: boolean;
  shiftKey?: boolean;
  handler: () => void;
  description: string;
}
```

---

### 2. 核心Hooks设计

#### `useVideoPlayer.ts`
```typescript
/**
 * 播放器核心Hook
 * 职责：管理播放器实例、事件监听、HLS配置
 */
interface UseVideoPlayerOptions {
  url: string;
  poster: string;
  title: string;
  containerRef: React.RefObject<HTMLDivElement>;
  blockAdEnabled: boolean;
  onReady?: () => void;
  onTimeUpdate?: (time: number, duration: number) => void;
  onEnded?: () => void;
}

export function useVideoPlayer(options: UseVideoPlayerOptions) {
  const playerRef = useRef<ArtPlayerInstance | null>(null);

  // 播放器初始化
  useEffect(() => {
    // 创建播放器
    // 注册事件
    // 返回清理函数
  }, [options.url]);

  return {
    player: playerRef.current,
    isReady: ...,
    // 控制方法
    play: () => {},
    pause: () => {},
    seek: (time: number) => {},
  };
}
```

#### `usePlaybackHistory.ts`
```typescript
/**
 * 播放记录Hook
 * 职责：保存/加载播放进度、自动恢复
 */
export function usePlaybackHistory(
  source: string,
  id: string,
  episodeIndex: number
) {
  // 加载历史记录
  useEffect(() => {
    // 从DB加载
  }, [source, id]);

  // 保存进度
  const saveProgress = useCallback((time: number, duration: number) => {
    // 节流保存到DB
  }, [source, id, episodeIndex]);

  return {
    resumeTime: ...,
    saveProgress,
  };
}
```

#### `useSourceSelection.ts`
```typescript
/**
 * 源选择Hook
 * 职责：搜索源、智能测速、换源逻辑
 */
export function useSourceSelection(
  searchTitle: string,
  searchType: string
) {
  const [sources, setSources] = useState<SearchResult[]>([]);
  const [loading, setLoading] = useState(false);

  // 搜索可用源
  const searchSources = useCallback(async () => {
    // 调用API搜索
    // 智能测速
    // 排序
  }, [searchTitle, searchType]);

  // 换源
  const switchSource = useCallback(async (
    newSource: string,
    newId: string
  ) => {
    // 保存当前状态
    // 切换源
    // 错误恢复
  }, []);

  return {
    sources,
    loading,
    searchSources,
    switchSource,
  };
}
```

#### `useKeyboardShortcuts.ts`
```typescript
/**
 * 键盘快捷键Hook
 * 职责：注册快捷键、处理键盘事件
 */
export function useKeyboardShortcuts(
  shortcuts: KeyboardShortcut[]
) {
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      // 匹配快捷键
      // 执行handler
    };

    document.addEventListener('keydown', handler);
    return () => document.removeEventListener('keydown', handler);
  }, [shortcuts]);
}
```

#### `useFavorite.ts`
```typescript
/**
 * 收藏功能Hook
 * 职责：收藏状态、添加/删除收藏
 */
export function useFavorite(source: string, id: string) {
  const [favorited, setFavorited] = useState(false);

  // 加载收藏状态
  useEffect(() => {
    // 从DB查询
  }, [source, id]);

  // 切换收藏
  const toggleFavorite = useCallback(async () => {
    // 添加/删除
  }, [source, id, favorited]);

  return { favorited, toggleFavorite };
}
```

#### `useVideoData.ts`
```typescript
/**
 * 视频数据Hook
 * 职责：加载视频详情、管理视频元数据
 */
export function useVideoData(
  initialSource: string,
  initialId: string,
  needPrefer: boolean
) {
  const [data, setData] = useState<VideoData>({...});
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // 加载视频详情
  useEffect(() => {
    // 根据needPrefer决定是否优选
    // 获取详情
    // 更新state
  }, [initialSource, initialId]);

  return { data, loading, error, updateData: setData };
}
```

---

### 3. 组件设计

#### `VideoPlayer/index.tsx`
```typescript
/**
 * 视频播放器组件
 * Props：配置、回调
 * 职责：渲染播放器容器、集成useVideoPlayer
 */
interface VideoPlayerProps {
  url: string;
  poster: string;
  title: string;
  blockAdEnabled: boolean;
  onTimeUpdate?: (time: number, duration: number) => void;
  onEnded?: () => void;
  onReady?: () => void;
}

export function VideoPlayer(props: VideoPlayerProps) {
  const containerRef = useRef<HTMLDivElement>(null);

  const { player } = useVideoPlayer({
    ...props,
    containerRef,
  });

  return <div ref={containerRef} className="..." />;
}
```

#### `VideoInfo/index.tsx`
```typescript
/**
 * 视频信息组件
 * Props：视频数据
 * 职责：展示标题、年份、简介、封面
 */
interface VideoInfoProps {
  title: string;
  year: string;
  cover: string;
  description?: string;
  className?: string;
  source?: string;
  sourceId?: string;
}

export function VideoInfo(props: VideoInfoProps) {
  const { favorited, toggleFavorite } = useFavorite(
    props.source || '',
    props.sourceId || ''
  );

  return (
    <div className={props.className}>
      <h1>{props.title}</h1>
      <FavoriteButton
        favorited={favorited}
        onClick={toggleFavorite}
      />
      {/* 其他信息 */}
    </div>
  );
}
```

#### `VideoInfo/FavoriteButton.tsx`
```typescript
/**
 * 收藏按钮组件
 * Props：状态、回调
 * 职责：渲染收藏图标、处理点击
 */
interface FavoriteButtonProps {
  favorited: boolean;
  onClick: () => void;
}

export function FavoriteButton({ favorited, onClick }: FavoriteButtonProps) {
  return (
    <button onClick={onClick}>
      {favorited ? <FilledHeart /> : <OutlineHeart />}
    </button>
  );
}
```

---

### 4. 主页面结构 (`page.tsx`)

```typescript
export default function PlayPage() {
  return (
    <Suspense fallback={<LoadingUI />}>
      <PlayPageErrorBoundary>
        <PlayPageClient />
      </PlayPageErrorBoundary>
    </Suspense>
  );
}

function PlayPageClient() {
  const searchParams = useSearchParams();

  // 使用各种Hooks
  const videoData = useVideoData(...);
  const playbackHistory = usePlaybackHistory(...);
  const sourceSelection = useSourceSelection(...);
  const keyboardShortcuts = useKeyboardShortcuts(...);

  // 简单的渲染逻辑
  return (
    <PageLayout>
      <VideoPlayer ... />
      <EpisodeSelector ... />
      <VideoInfo ... />
      <SkipController ... />
    </PageLayout>
  );
}
```

**主页面只负责**:
- 获取URL参数
- 组装Hooks
- 渲染组件
- 传递props

---

## 🚀 实施步骤

### Phase 1: 基础设施（30分钟）
1. ✅ 创建types文件
2. ✅ 创建utils文件
3. ✅ 创建错误边界（已完成）

### Phase 2: Hooks层（1小时）
4. ✅ useVideoPlayer
5. ✅ usePlaybackHistory
6. ✅ useSourceSelection
7. ✅ useKeyboardShortcuts
8. ✅ useFavorite
9. ✅ useVideoData

### Phase 3: 组件层（45分钟）
10. ✅ VideoPlayer组件
11. ✅ VideoInfo组件
12. ✅ FavoriteButton组件
13. ✅ AdFilterLoader

### Phase 4: 主页面（30分钟）
14. ✅ 重写PlayPageClient
15. ✅ 集成所有模块
16. ✅ 清理旧代码

### Phase 5: 测试验证（30分钟）
17. ✅ 功能测试
18. ✅ 类型检查
19. ✅ 构建测试

**预计总时间**: 3-3.5小时

---

## ✅ 质量标准

### 代码质量
- [ ] 无 TypeScript 错误
- [ ] 无 ESLint 警告
- [ ] 所有函数有JSDoc注释
- [ ] 关键逻辑有代码注释

### 功能完整性
- [ ] 所有原有功能正常
- [ ] 播放器正常工作
- [ ] 换源功能正常
- [ ] 播放记录正常
- [ ] 收藏功能正常
- [ ] 快捷键正常

### 性能
- [ ] 无内存泄漏
- [ ] 无不必要的重渲染
- [ ] 加载速度不变慢

---

## 📊 预期收益

### 短期收益
- ✅ 代码结构清晰
- ✅ 易于理解和修改
- ✅ 类型安全，减少bug

### 长期收益
- ✅ 易于维护和扩展
- ✅ 组件可复用
- ✅ 便于单元测试
- ✅ 新人容易上手
- ✅ Git冲突减少

### 技术债务
- ✅ 消除1900行单文件
- ✅ 消除 any 类型
- ✅ 消除重复代码

---

**开始时间**: 2025-11-12
**预计完成**: 3-3.5小时
**负责人**: Claude Code
