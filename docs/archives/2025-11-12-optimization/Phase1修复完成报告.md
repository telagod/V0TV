...
- `https://vip.dytt-cinema.com/20250215/xxx/index.m3u8` ← **真实M3U8**（✅ 可播放）
...
**修复效果**:
- ✅ 支持多个播放源，自动选择最优
- ✅ dyttm3u8源（优先级1）排在dytt源（优先级99）前面
- ✅ 用户可以在多个播放源之间切换
- ✅ 某个源失效时自动切换备用源
...
**分析**:
- **dytt源**: 0个.m3u8链接（使用/share/中转，已被过滤）
- **dyttm3u8源**: 173个标准m3u8链接（✅ 可用）
...
**修复前的行为** ❌:
1. searchFromApi 第一页: 使用 `$$$` 分割，可能提取到dytt源
...
2. 检测到2个播放源: dytt 和 dyttm3u8 ✅
3. dytt源: 0个有效链接（全是/share/，被过滤）
4. dyttm3u8源: 173个有效链接 ✅
5. 自动选择 dyttm3u8源（优先级1） ✅
6. 返回标准格式链接，可直接播放 ✅
